...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_NEWTHREAD:
		int start_function = current->p_registers.reg_eax;
		int pid = 1;
		process_t* proc;
		for(; 
		while (1)
		{
			if(pid == NPROCS)
				proc->p_registers.reg_eax = -1;
			proc = &(proc_array[pid]);
			if (proc->p_state == P_EMPTY) break;
			pid+=1;
		}
		proc->p_state = P_RUNNABLE;
		proc->p_registers.reg_esp = (uint32_t)(PROC1_STACK_ADDR + (PROC_STACK_SIZE * avail->p_pid));	
...
do_fork(process_t *parent)
{
	...
	if(pid == NPROCS) return -1;
	process_t *newproc = &proc_array[pid];	
	//void *memcpy(void *dest, const void *src, size_t n);
	memcpy(newproc, parent, sizeof(process_t));
	newproc->p_pid = pid;
	newproc->p_registers.reg_eax = 0; 
	newproc->p_state = P_RUNNABLE;
	//
	copy_stack(newproc,parent);
...
copy_stack(process_t *dest, process_t *src)
{
	...
	uint32_t src_stack_size = src_stack_top - src_stack_bottom;	

	dest_stack_top = PROC1_STACK_ADDR + PROC_STACK_SIZE * dest->p_pid;
	dest_stack_bottom = dest_stack_top - src_stack_size;
	dest->p_registers.reg_esp = dest_stack_bottom;
	// memcpy the stack and set dest->p_registers.reg_esp

	memcpy ((void*)dest_stack_bottom, (void*)src_stack_bottom, src_stack_size);
}
